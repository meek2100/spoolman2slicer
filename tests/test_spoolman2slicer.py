...
# Create minimal required template files with the tool signature
...
# Create a default args object for tests that expect it
_default_args = Mock()
_default_args.dir = tempfile.mkdtemp()
_default_args.slicer = Slicers.SUPERSLICER
_default_args.verbose = False
    ...
    def test_load_filaments_connection_error(self):
        ...
        with patch("requests.get", side_effect=requests.exceptions.ConnectionError):
            with pytest.raises(requests.exceptions.ConnectionError):
                spoolman2slicer.load_filaments_from_spoolman("http://test.local:8000")
        ...
        with (
            ...
            with pytest.raises(requests.exceptions.ConnectionError):
                spoolman2slicer.load_filaments_from_spoolman(
                    "http://test.local:8000", max_retries=3
                )
...
class TestFileOperations:
    ...
    def test_delete_filament(
        ...
        with (
            ...
            # Ensure template has signature for safety check
            Path(temp_template_dir, "default.ini.template").write_text(
                "# generated by spoolman2slicer\nfilament_type = {{material}}\n"
            )
...
class TestAddSm2sToFilament:
    ...
    def test_add_sm2s_data(self, sample_filament_data):
        ...
        with (
            ...
            assert (
                sample_filament_data["sm2s"]["spoolman_url"] == "http://test.local:8000"
            )
...
class TestDeleteAll:
    ...
    def test_delete_all_filaments(self, temp_output_dir):
        ...
        # Create some test files with the tool signature to allow deletion
        sig = "# generated by spoolman2slicer\n"
        (Path(temp_output_dir) / "test1.ini").write_text(sig + "content1")
        (Path(temp_output_dir) / "test2.ini").write_text(sig + "content2")
    ...
    def test_delete_all_preserves_manual_profiles(self, temp_output_dir):
        """Test that non-managed files are NOT deleted"""
    ...
    def test_create_per_spool_all_mode(self, temp_template_dir, temp_output_dir):
        ...
        with (
            ...
            with patch.object(
                ...
                spoolman2slicer.load_and_update_all_filaments("http://test.local:8000")
...
class TestArgumentParsing:
    """Test argument parsing and environment variable mapping"""

    def test_url_env_mapping(self):
        """Test that SM2S_SPOOLMAN_URL is picked up for --url"""
        with patch.dict(os.environ, {"SM2S_SPOOLMAN_URL": "http://env-url:8000"}):
            with patch("sys.argv", ["spoolman2slicer.py", "--dir", "/tmp"]):
                args_obj, _ = spoolman2slicer.parse_args()
                assert args_obj.url == "http://env-url:8000"

    def test_url_legacy_env_mapping(self):
        """Test that legacy SPOOLMAN_URL is picked up for --url"""
        with patch.dict(os.environ, {"SPOOLMAN_URL": "http://legacy-url:8000"}):
            # Ensure SM2S_SPOOLMAN_URL is NOT set
            if "SM2S_SPOOLMAN_URL" in os.environ:
                del os.environ["SM2S_SPOOLMAN_URL"]
            with patch("sys.argv", ["spoolman2slicer.py", "--dir", "/tmp"]):
                args_obj, _ = spoolman2slicer.parse_args()
                assert args_obj.url == "http://legacy-url:8000"

    def test_url_cli_overrides_env(self):
        """Test that CLI argument overrides environment variables"""
        env = {
            "SM2S_SPOOLMAN_URL": "http://env-url:8000",
            "SPOOLMAN_URL": "http://legacy-url:8000",
        }
        with patch.dict(os.environ, env):
            with patch(
                "sys.argv",
                ["spoolman2slicer.py", "--dir", "/tmp", "--url", "http://cli-url:8000"],
            ):
                args_obj, _ = spoolman2slicer.parse_args()
                assert args_obj.url == "http://cli-url:8000"

    def test_slicer_env_mapping(self):
        """Test that SM2S_SLICER is picked up"""
        with patch.dict(os.environ, {"SM2S_SLICER": "orcaslicer"}):
            with patch("sys.argv", ["spoolman2slicer.py", "--dir", "/tmp"]):
                args_obj, _ = spoolman2slicer.parse_args()
                assert args_obj.slicer == Slicers.ORCA

    def test_slicer_env_sanitization(self):
        """Test that environment variables are sanitized (quotes stripped)"""
        with patch.dict(os.environ, {"SM2S_SLICER": '"orcaslicer"'}):
            with patch("sys.argv", ["spoolman2slicer.py", "--dir", "/tmp"]):
                args_obj, _ = spoolman2slicer.parse_args()
                assert args_obj.slicer == Slicers.ORCA

    def test_dir_env_mapping(self):
        """Test that SM2S_SLICER_CONFIG_DIR is picked up"""
        with patch.dict(os.environ, {"SM2S_SLICER_CONFIG_DIR": "/env/config/dir"}):
            with patch("sys.argv", ["spoolman2slicer.py"]):
                args_obj, _ = spoolman2slicer.parse_args()
                assert args_obj.dir == "/env/config/dir"

    def test_boolean_env_mapping(self):
        """Test that boolean environment variables are mapped correctly"""
        env = {
            "SM2S_LIVE_SYNC": "true",
            "SM2S_VERBOSE_LOGGING": "1",
            "SM2S_STARTUP_TIDY": "yes",
        }
        with patch.dict(os.environ, env):
            with patch("sys.argv", ["spoolman2slicer.py", "--dir", "/tmp"]):
                args_obj, _ = spoolman2slicer.parse_args()
    ...
    def test_invalid_slicer_env_returns_error(self):
        ...
        with patch.dict(os.environ, {"SM2S_SLICER": "invalid_slicer"}):
            with patch("sys.argv", ["spoolman2slicer.py", "--dir", "/tmp"]):
                # Mock parser.error to prevent sys.exit
                with patch("argparse.ArgumentParser.error") as mock_error:
                    spoolman2slicer.parse_args()
                    assert mock_error.called
                    assert (
                        "invalid choice" in mock_error.call_args[0][0].lower()
                        or "choose from" in mock_error.call_args[0][0].lower()
                    )

    def test_slicer_suffix_generation(self):
        """Test that get_config_suffix returns correct values for different slicers"""
        with patch(
